...
class Consumer(threading.Thread):
	...
	self.device.write("""n0.val=%d\xff\xff\xff"""%totallayer) 
		...
		if result==-2:
	            break
		if result==-1:
	            self.device.write("""page 6\xff\xff\xff""")
		    break
...
class Control(threading.Thread):
    ...
    def start(self):
        self.ui_control(self.port,self.boud)
    
    def ui_control(self,port,boud):
        speed=0
        upheight=0
        slice_path=""
        print_path=""
        ...
        device.flushInput()
        device.flushOutput()
        device.write("""page 0\xff\xff\xff""")
        while True:
	    try:
                str=device.read(6)
	        ...
	        if result[0]=='00':
		        ...
		        if sliceorprint==2:
			        ...
			        print print_file
                        ...
                        if sliceorprint == 1:
                            ...
                            totalfile=os.path.join(slice_path,postfile)
                            slicefile=totalfile
                            device.write("""page 7\xff\xff\xff""")
                            device.write("""t1.txt="%s"\xff\xff\xff"""% postfile)
                        if sliceorprint == 2:
                            postfile=print_file[int(result[2],16)]
                            totalfile=os.path.join(print_path,postfile)
                            printfile=totalfile
                            device.write("""page 10\xff\xff\xff""")
                            device.write("""t1.txt="%s"\xff\xff\xff"""%postfile)
                    elif var=='02':
                        if sliceorprint==1:
                        ...
                        device.write("""page 3\xff\xff\xff""")
                    elif var=='0e':    
                        t0=Print(printfile,timeinterval,upheight)
		        ...
		        print_file=[]
                        ...
                        device.write("""vis b1,0\xff\xff\xff""")
                        device.write("""vis b0,0\xff\xff\xff""")
                        device.write("""vis b3,0\xff\xff\xff""")
                        Gohome()
                        device.write("""vis b1,1\xff\xff\xff""")
                        device.write("""vis b0,1\xff\xff\xff""")
